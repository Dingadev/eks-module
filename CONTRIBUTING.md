...
functionality you're adding in your contribution. Check out the [tests](https://github.com/gruntwork-io/terraform-aws-eks/tree/master/test) folder for instructions on running the 
automated tests. 
...
You must run `terraform fmt` on the code before committing and `yapf` on the python code. You can configure your
computer to do this automatically using pre-commit hooks managed using [pre-commit](http://pre-commit.com/):

1. [Install pre-commit](http://pre-commit.com/#install). E.g.: `brew install pre-commit`.
1. Install the hooks: `pre-commit install`.
1. Install the required hook binaries: `pip install yapf`

That's it! Now just write your code, and every time you commit, `terraform fmt` and `yapf` will be run on the files
you're committing.
